...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Privacy Policy"
            : footer?.data?.meta_title_ar ?? "سياسة الخصوصية | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Privacy Policy"
            : footer?.data?.meta_description_ar ??
            "سياسة الخصوصية معارض تمكين";

    // In /[lang]/privacy-policy/[...slug], slugParts are ONLY the [...slug] bits (not "privacy-policy")
