...
import React, { useContext, useEffect, useRef, useState, use } from 'react'
...
export default function Register(
    ...
    const getNotificationData = async () => {
        const getNotifyData = await getSignUpData();
        setNotificationsListing(getNotifyData.latestnotification?.data || []);
    }
    ...
    const options = [
        { value: '1', label: lang === 'ar' ? 'ذكر' : 'Male' },
        { value: '0', label: lang === 'ar' ? 'أنثى' : 'Female' }
    ]
    ...
    const CheckRegisterOtp = async () => {
        try {
            ...
            const res = await postCheckRegisterOtpData(dataotp);
            if (res?.usercheckRegister?.success === true) {
                Register()
    ...
    const CheckRegisterPhone = async () => {
        try {
            ...
            const res = await postCheckRegisterCheckData(data);
            if (res?.usercheckRegisterData?.success === true) {
                setOtpBox(true)
                setLoginBtnLoading(false)
            }
            else {
    ...
    const Register = async () => {
        try {
            ...
            const res = await postSignupUser(data);
            if (res?.userRegister?.success === true) {
                localStorage.setItem("userid", res?.userRegister?.userNew?.id.toString())
                localStorage.setItem('eMail', res?.userRegister?.userNew?.email.toString())
                localStorage.setItem('fullName', res?.userRegister?.userNew?.full_name.toString())
                localStorage.setItem('phoneNumber', res?.userRegister?.userNew?.phone_number.toString())
                localStorage.setItem('loyaltyCount', res?.userRegister?.userNew?.loyaltypoints.toString())
                localStorage.setItem('compareCount', res?.userRegister?.userNew?.compares_count.toString())
                localStorage.setItem('wishlistCount', res?.userRegister?.userNew?.wishlists_count.toString())
                localStorage.setItem('orderCount', res?.userRegister?.userNew?.confirmed_orders_data_count.toString())
                // Global State Update
                ...
                topMessageAlartSuccess(t('login.Register'))

                const status = res?.userRegister?.error
                    ? `Signup Failed: ${res?.userRegister?.error.message || 'Unknown error'}`
                    : 'Signup Success';
                ...
                const fullName = res?.userRegister?.userNew?.full_name?.toString() || '';
                ...
                window.dataLayer.push({
                    ...
                    platform: deviceType,
                    gender,
                    email: res?.userRegister?.userNew?.email?.toString() || '',
                    phone: `966${res?.userRegister?.userNew?.phone_number?.toString() || ''}`,
                ...
                if (searchParams?.type) {
                    ...
                    router.push(`${origin}/${lang}?refresh=${Math.random()}`, { scroll: false })
                }
                router.refresh()
            }
            else {
            ...
            <div className="container py-16">
                <div className="pt-2">
                    ...
                    {otpbox == true ?
                        <>
                            ...
                            <p className="text-xs font-normal mt-1 text-[#5D686F] text-center">{lang == 'ar' ? 'تم إرسال OTP إلى' : 'OTP has been sent to'} <br /><span className="text-[#004B7A]">+966{phoneNumWithoutDash}</span> <br /> {lang == 'ar' ? 'عبر الرسائل القصيرة' : 'via SMS'}</p>
                        ...
                        <>
                            {/* REGISTER BOX */}
                            <h1 className="text-xl font-bold">{lang == 'ar' ? 'جديد معنا؟ سجل الآن' : 'New to TamkeenStores? Register!'}</h1>
                            <p className="text-xs font-normal mt-1 text-[#5D686F]">{lang == 'ar' ? 'أو سجل عبر بريدك الإلكتروني' : 'Sign up using email'}</p>
                            <div className="mt-4">
                                <div className="mb-1">
                                    ...
                                    <div className="pb-3 pt-2.5 px-3 bg-white rounded-md border flex items-center border-[#5D686F30] gap-x-2">
                                        <input className="focus-visible:outline-none w-1/2 text-xs font-normal" type="text" placeholder={lang === 'ar' ? 'الاسم الأول' : 'First Name'}
                                            value={firstName}
                                            onChange={(e: any) => {
                                        ...
                                        <input className="focus-visible:outline-none w-1/2 text-xs font-normal" type="text" placeholder={lang === 'ar' ? 'اسم العائلة' : 'Last Name'}
                                            value={lastName}
                                            onChange={(e: any) => {
                                ...
                                <div className="mb-1 flex items-center justify-between gap-x-3">
                                    <div className="w-1/2">
                                        <label className="text-xs font-normal">{lang === 'ar' ? 'النوع (اختياري) ' : 'Select Gender'}</label>
                                        <Select
                                            styles={{
                                    ...
                                    <div className='w-1/2'>
                                        <label className="text-xs font-normal">{lang === 'ar' ? 'تاريخ الميلاد (اختياري)' : 'Date of Birth'}</label>
                                        <div className="pb-3.5 pt-3 px-3 bg-white rounded-md border border-[#5D686F30] w-full">
                                            <input className="focus-visible:outline-none w-full text-xs font-normal" placeholder={lang === 'ar' ? 'تاريخ الميلاد (اختياري)' : 'Date of Birth'} type="date"
                                                value={dateOfBirth}
                                                onChange={(e) => {
                                ...
                                <div className="mb-1">
                                    ...
                                    <div className="pb-3.5 pt-3 px-3 bg-white rounded-md border flex items-center border-[#5D686F30] gap-x-2">
                                        <input className="focus-visible:outline-none w-1/2 text-xs font-normal" placeholder={lang === 'ar' ? 'البريد الإلكتروني' : 'Email ID'} type="email"
                                            value={email}
                                            onChange={(e: any) => {
                ...
                <div className="w-full mt-10">
                    <div className="bg-white rounded-md shadow-md">
                        <div>
                            <Link href={`${origin}/${lang}/${notificationsListing?.link}`} className="focus-visible:outline-none w-full text-left">
                                {notificationsListing?.image ?
                                    <Image loading='lazy' src={notificationsListing?.image} alt={lang === 'ar' ? notificationsListing?.title_arabic : notificationsListing?.title} title={lang === 'ar' ? notificationsListing?.title_arabic : notificationsListing?.title} height={0} width={0} className="h-auto w-full rounded-tl-md rounded-tr-md" />
                                    :
                                ...
                                <div className="px-5 pt-3">
                                    <h6 className="font-semibold text-base text-[#004B7A]">{lang === 'ar' ? notificationsListing?.title_arabic : notificationsListing?.title}</h6>
                                    <small className="font-medium text-[#000000]">{lang === 'ar' ? notificationsListing?.message_arabic : notificationsListing?.message}</small>
                        ...
                        <div className="px-5 pb-5">
                            ...
                            <div className="mt-4">
                                <h3 className="font-semibold text-sm">{lang === 'ar' ? 'هل أنت جديد في متاجر تمكين؟ يسجل!' : 'New to Tamkeen Stores? Register!'}</h3>
                                <div className="mt-4">
