...
export default async function PormotionLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Pormotion"
            : footer?.data?.meta_title_ar ?? "عروض | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Pormotion"
            : footer?.data?.meta_description_ar ??
            "عروض معارض تمكين";

    // In /[lang]/pormotion/[...slug], slugParts are ONLY the [...slug] bits (not "pormotion")
