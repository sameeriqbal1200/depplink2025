...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Login"
            : footer?.data?.meta_title_ar ?? "تسجيل الدخول | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Login"
            : footer?.data?.meta_description_ar ??
            "تسجيل الدخول معارض تمكين";

    // In /[lang]/login/[...slug], slugParts are ONLY the [...slug] bits (not "login")
