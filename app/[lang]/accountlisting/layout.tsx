...
export default async function AccountListingLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Account Listing"
            : footer?.data?.meta_title_ar ?? "قائمة الحسابات | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Account Listing"
            : footer?.data?.meta_description_ar ??
            "قائمة الحسابات معارض تمكين";
