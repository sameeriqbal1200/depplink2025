...
export default async function TamaraLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Tamara"
            : footer?.data?.meta_title_ar ?? "الأسئلة الشائعة | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Tamara"
            : footer?.data?.meta_description_ar ??
            "تمارا معارض تمكين";

    // In /[lang]/Tamara/[...slug], slugParts are ONLY the [...slug] bits (not "Tamara")
