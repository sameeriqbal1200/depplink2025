...
export default async function TamkeenWalletLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Tamkeen Wallet"
            : footer?.data?.meta_title_ar ?? "محفظة تمكين | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Tamkeen Wallet"
            : footer?.data?.meta_description_ar ??
            "معارض تمكين محفظة تمكين";

    // In /[lang]/warranty-policy/[...slug], slugParts are ONLY the [...slug] bits (not "Tamkeen Wallet")
