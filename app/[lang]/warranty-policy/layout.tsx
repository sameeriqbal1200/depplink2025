...
export default async function WarrentyPolicyLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Warranty Policy"
            : footer?.data?.meta_title_ar ?? "سياسة الضمان | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Warranty Policy"
            : footer?.data?.meta_description_ar ??
            "معارض تمكين سياسة الضمان";

    // In /[lang]/warranty-policy/[...slug], slugParts are ONLY the [...slug] bits (not "Warranty Policy")
