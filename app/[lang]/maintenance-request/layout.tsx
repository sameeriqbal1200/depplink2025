...
export default async function ServiceAppointmentLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "This is Maintaince Request Page"
            : footer?.data?.meta_title_ar ?? 'هذه صفحة طلب الصيانة';

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "This is Maintaince Request Page"
