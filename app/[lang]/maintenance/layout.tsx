...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Maintenance"
            : footer?.data?.meta_title_ar ?? "الصيانة | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Maintenance"
            : footer?.data?.meta_description_ar ??
            "الصيانة معارض تمكين";

    // In /[lang]/maintenance/[...slug], slugParts are ONLY the [...slug] bits (not "maintenance")
