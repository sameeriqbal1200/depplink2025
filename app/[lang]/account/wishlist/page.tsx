  ...
  <div className="flex h-16 items-center justify-center gap-2 text-[#004B7A] my-22">
    {state === "pulling" ? (
      <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
        <path d="M12 3a1 1 0 0 1 1 1v11.586l3.293-3.293 1.414 1.414-5 5a1 1 0 0 1-1.414 0l-5-5 1.414-1.414L11 15.586V4a1 1 0 0 1 1-1z" />
      </svg>
    ) : (
      <div
  ...
  return (
    <div>
      <MobileHeader
        type="Third"
        lang={lang}
        pageTitle={lang === "ar" ? "اخر طلباتك" : "Wishlist"}
      />
      {/* <PullToRefresh
                onRefresh={onRefresh}
        ...
        {loading ? (
          ...
          <div className="my-6 grid grid-cols-1 gap-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="h-40 bg-gray-100 rounded-md animate-pulse"
              />
            ))}
          </div>
        ) : (
          <div className="flex items-start my-4 gap-x-5">
            <div className="w-full">
              <h2 className="mb-4 font-bold text-base max-md:hidden">
                {lang === "ar" ? "قائمة المفضلة" : "Wishlist"} ({count})
              </h2>

              {!isEmpty ? (
                <div className="tamkeenSales_cardss relative grid  grid-cols-1 gap-2 items-start justify-center">
                  <ProductLoop
                    productData={items}
                    lang={isArabic} // or pass isArabic if ProductLoop expects boolean
