...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Categories Listing"
            : footer?.data?.meta_title_ar ?? "قائمة الفئات | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Categories Listing"
            : footer?.data?.meta_description_ar ??
            "قائمة الفئات معارض تمكين";

    // In /[lang]/categorieslisting/[...slug], slugParts are ONLY the [...slug] bits (not "categorieslisting")
