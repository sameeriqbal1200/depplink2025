...
export default function Category({ params, searchParams }: { params: { data: any }, searchParams: any }) {
  ...
  const filter = () => {
    ...
    if (Object.keys(filterdata).length >= 1) {
      router.push(`/${lang}/most-rated${result}`, { scroll: false })
      router.refresh();
    }
  }
            ...
            {CatData?.productData?.products?.data?.length ?
              <div className="px-0 flex-1 py-0 ltr:xl:mr-1 rtl:xl:ml-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-3">
                  {/* SORTING */}
                  <Products isArabic={isArabic} NewMedia={NewMedia} lang={lang} dict={dict?.products} products={CatData?.productData?.products?.data} />
                </div>
              ...
              <div className="container my-10 flex items-center justify-center py-8">
                <div className='text-center'>
                  ...
                  <button className="btn focus-visible:outline-none bg-[#004B7A] w-72 p-2.5 rounded-md text-sm 2xl:text-base text-white mt-6" onClick={() => router.push('/')}>{lang == 'ar' ? 'تسوق المنتجات' : 'Shop products'}</button>
                </div>
              </div>
            }
          </div>
