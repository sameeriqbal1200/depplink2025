...
export default async function TermsAndConditionsLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Terms and Conditions"
            : footer?.data?.meta_title_ar ?? "الشروط والأحكام | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Terms and Conditions"
            : footer?.data?.meta_description_ar ??
            "معارض تمكين الشروط والأحكام";

    // In /[lang]/warranty-policy/[...slug], slugParts are ONLY the [...slug] bits (not "Terms and Conditions")
