    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === "ar" ? "مقارنة المنتجات" : "Compare Products",
                    item: `${baseUrl}/${lang}/compare`,
                },
            ],
        },
    ];
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Compare Products"
            : footer?.data?.meta_title_ar ?? "مقارنة المنتجات | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ?? "Compare products at Tamkeen Stores. Easily evaluate features, prices, and specifications to make the best choice."
            : footer?.data?.meta_description_ar ?? "قارن المنتجات في معارض تمكين بسهولة، وقيم المواصفات والأسعار والخصائص لاختيار الأنسب.";
