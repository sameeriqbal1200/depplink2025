...
export default async function TamkeenPremiumBuyLayout({ children }: { children: React.ReactNode }) {
    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === 'ar' ? 'شراء تمكين بريميوم' : 'Tamkeen Premium Buy',
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Tamkeen Premium Buy"
            : footer?.data?.meta_title_ar ?? "شراء تمكين بريميوم | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ?? "Tamkeen Stores Tamkeen Premium Buy"
            : footer?.data?.meta_description_ar ?? "شراء تمكين بريميوم معارض تمكين";

    // In /[lang]/projectsales/[...slug], slugParts are ONLY the [...slug] bits (not "projectsales")
