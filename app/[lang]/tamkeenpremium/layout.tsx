...
export default async function TamkeenPremiumLayout({ children }: { children: React.ReactNode }) {
    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === 'ar' ? 'تمكين بريميوم' : 'Tamkeen Premium',
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Tamkeen Premium"
            : footer?.data?.meta_title_ar ?? "تمكين بريميوم | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ?? "Tamkeen Stores Tamkeen Premium"
            : footer?.data?.meta_description_ar ?? "تمكين بريميوم معارض تمكين";
