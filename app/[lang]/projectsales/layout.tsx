...
export default async function ProjectSalesLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Project Sales"
            : footer?.data?.meta_title_ar ?? "الأسئلة الشائعة | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Project Sales"
            : footer?.data?.meta_description_ar ??
            "معارض تمكين الأسئلة الشائعة";
