...
export default async function InstallmetnServiceMethodsLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Installment Service Methods"
            : footer?.data?.meta_title_ar ?? "طرق خدمة التقسيط | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Installment Service Methods"
            : footer?.data?.meta_description_ar ??
            "طرق خدمة التقسيط معارض تمكين";

    // In /[lang]/installment-service-methods/[...slug], slugParts are ONLY the [...slug] bits (not "installment-service-methods")
