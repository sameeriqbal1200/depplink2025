...
export default function Checkout() {
    ...
    // CURRENCY SYMBOL //
    const currencySymbol =<SARIcon size={11} />

    const getCustomerAddressData = async () => {
    if (localStorage.getItem("userid")) {
      getCitiesData(lang);
      const addressData = await getUserAddressData();
      const responseJson = addressData?.addressData
    ...
    const getUser = () => {
        if (localStorage.getItem("userid")) {
        setEmail(localStorage.getItem("eMail")); // How to call the localStorage
        setPhoneNumber(localStorage.getItem("phoneNumber")); // How to call the localStorage
        setFullName(localStorage.getItem("fullName")); // How to call the localStorage
        } else {
        router.push(`${origin}/${lang}/login?type=checkout`);
        }
    };
    ...
    const setGTMBBeginCheckout = () => {
        ...
        return {
            "item_id": product?.sku ?? "",
            "item_name": lang == 'ar' ? (product?.name_arabic ?? "") : (product?.name ?? ""),
            "item_availability": "in stock",
            "item_brand": lang == 'ar' ? (product?.brand?.name_arabic ?? "") : (product?.brand?.name ?? ""),
            "item_image_link": product?.image ?? "",
            "item_link": `${origin}/${lang}/product/${product?.slug ?? ""}`,
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!recheck?.success) {
                topMessageAlartDangerNew(lang == 'ar' ? 'هناك بعض التحديثات في سلة التسوق الخاصة بك.' : 'There is some updates in your cart.')
    ...
    const getPickupStoreData = async () => {
        const warehouseData: any = await getwarehouseData(lang);
        const responseJson = warehouseData?.warehouseData
    ...
    const couponremove = async () => {
        unsetcoupon()
        setcouponcode('')
        topMessageAlartDangerNew(t('unsetcoupon'))
        var city = false;
        var selectedaddres = addressData?.filter((element: any) => {
            return element.id == addressid
        })
    ...
    const DataLayerCheckout = () => {
        ...
        for (let index = 0; index < checkoutData?.products?.length; index++) {
            ...
            var obj: any = {};
            obj["id"] = element?.sku;
            obj["name"] = lang == "ar" ? element?.name_arabic : element?.name;
        ...
        wind.push({
            ...
            address: {
                ...
                city: lang == 'ar' ? address?.state_data?.name_arabic : address?.state_data?.name,
    ...
    const submitOrder = async () => {
        ...
        var recheck = await recheckcartdata(lang, city)
        if (recheck.success && getSubtotalSale() > 0) {
            ...
            var response: any = await proceedToCheckout(city, lang, userDevice)
    ...
    const addressData = await addAddressData(data);
    const responseJson = addressData?.addressData
    if (responseJson?.success) {
      setAddAddress(false), setLoader(false);
      ...
      setLoader(false);
      topMessageAlartDangerNew(t('somethingwentwrong'));
    }
    ...
    const UpdateAddress = async (dataid: any) => {
        ...
        const addressData = await updateAddressData(dataid, data);
        const responseJson = addressData?.addressData
        if (responseJson?.success) {
        setLoader(false);
        getCustomerAddressData();
    ...
    async function getCitiesData(lang: string) {
        const citydata: any = await getcityData(lang);
        const responseJson = citydata?.cityData
        setCitiesData(responseJson?.data);
        var selectcity = responseJson?.data?.filter(
    ...
    const paymentmethods = [
        ...
        },
        {
            key: 'tamara',
            name: 'Tamara',
            image: lang == 'ar' ? '/images/tamara-ar.webp' : '/images/tamara-en.webp'
        },
        {
            key: 'tabby',
            name: 'Tabby',
            image: lang == 'ar' ? '/images/tabby-ar.webp' : '/images/tabby-en.webp'
        },
        {
            key: 'tasheel',
            name: 'Baseeta',
    ...
    return (
        <>
            ...
            <MobileHeader type="Third" lang={lang} redirect="cart" pageTitle={lang === 'ar' ? 'الدفع واكمال الطلب' : 'Checkout'} />
                ...
                <div className="md:flex items-start my-4 gap-x-5">
                    <div className="w-full">
                        {activeTab3 === 1 ?
                            <>
                                <div className="my-4">
                                    <h3 className="text-xl  font-semibold mb-3">{lang == 'ar' ? 'هلا!' : 'Hi!'} {fullName}</h3>
                                    <div className="flex items-start my-4 gap-x-5">
                                        <div className={`w-full ${addAddress == true ? 'block' : 'hidden'}`}>
                                            <div className="flex items-center justify-between mb-1 font-bold text-base mt-4">
                                                <h2>{lang == 'ar' ? 'اضف عنوان' : 'Add Address'}</h2>
                                                {addressData?.length ?
                                                    <button onClick={() => {
                                                        setAddAddress(false),
                                                            setProceesBtn(false)
                                                    }} className='focus-visible:outline-none text-[#219EBC] hover:underline text-sm'>{lang == 'ar' ? 'خلف' : 'Back'}</button>
                                                    : null}
                                            </div>
                                            <div className={`pb-3 ${addAddress == true ? 'block' : 'hidden'}`}>
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <label htmlFor="deafultAddress" className="focus-visible:outline-[#004B7A] fill-primary text-sm w-full items-center flex gap-x-2">
                                                        ...
                                                        {lang == 'ar' ? 'اجعله عنوانًا افتراضيًا' : 'Make as Primary?'}
                                                    </label>
                                    ...
                                    {deliOrder === true ?
                                        ...
                                        <div className={`mt-4 ${addAddress == true ? 'hidden' : 'block'}`}>
                                            <span className="text-[#B15533] text-sm">{lang == 'ar' ? ' تفاصيل العنوان' : 'Address Details'}</span>
                                            {addressid ? (
                                            <div className="mt-2">
                                                                        ...
                                                                        <div className="flex items-center gap-x-3 w-full">
                                                                            ...
                                                                            <div className="w-full">
                                                                                <RadioGroup.Label as="p" className={`text-sm font-bold  mb-3 text-white`}>
                                                                                    {addressData?.filter((item: any) => item?.id == addressid)[0]?.address_label == 0 ? lang == 'ar' ? 'الــمنـــزل' : 'Home' : lang == 'ar' ? 'مكتب' : 'Office'}
                                                                                    {addressData?.filter((item: any) => item?.id == addressid)[0]?.make_default == 1 ?
                                                                                        <span className={`px-2 py-1 bg-[#219EBC30] text-xs rounded-sm text-[#219EBC] ml-2 rtl:mr-2`}>{lang == 'ar' ? 'العنوان الرئيسي' : 'Primary Address'}</span>
                                                                                        : null}
                                                                                </RadioGroup.Label>
                                                                                <p className={`mt-1 text-xs text-white`}>{addressData?.filter((item: any) => item?.id == addressid)[0]?.address}</p>
                                                                                <p className={`mt-1.5 text-xs font-bold text-white`}>{lang == 'ar' ? addressData?.filter((item: any) => item?.id == addressid)[0]?.state_data?.region?.name_arabic : addressData?.filter((item: any) => item?.id == addressid)[0]?.state_data?.region?.name}, {lang == 'ar' ? addressData?.filter((item: any) => item?.id == addressid)[0]?.state_data?.name_arabic : addressData?.filter((item: any) => item?.id == addressid)[0]?.state_data?.name} | {lang == 'ar' ? 'المملكة العربية السعودية' : 'Saudi Arabia'}</p>
                                                                                {addressData?.filter((item: any) => item?.id == addressid)[0]?.shippinginstractions ?
                                                                                    <div className="mt-3">
                                                                                        <p className={`mt-3 text-xs font-bold text-white`}>{lang == 'ar' ? 'تعليمات الشحن' : 'Shipping Instructions'}:</p>
                                                ...
                                                //     router.push(`/${lang}/account/addressbook?addAddressCheckout=true`)
                                                // } 
                                                className={`focus-visible:outline-none underline text-sm mt-3 text-primary hover:text-[#B15533] rtl:pl-3 ltr:pr-3`}>+ {lang == 'ar' ? 'اضــافــة عــنوان جديد' : 'Add a new address'}
                                                </button>
                                                <button
                                                onClick={() => {
                                                    setchangeAddressPopup(true)
                                                }}
                                                    className="focus-visible:outline-none underline text-sm mt-3 text-primary hover:text-[#B15533]">{lang == 'ar' ? 'تغيير العنوان' : 'Change address'}
                                                </button>
                                                </>
                                                :
                                                <>
                                                                    ...
                                                                    <>
                                                                        <div className="flex w-full items-center justify-between">
                                                                            <div className="flex items-center gap-x-3 w-full">
                                                                                ...
                                                                                <div className="w-full">
                                                                                    <RadioGroup.Label as="p" className={`text-sm font-bold  mb-3 text-white`}>
                                                                                    {lang == 'ar' ? 'متجر الاستلام' : 'Collect From Store'}
                                                                                    </RadioGroup.Label>
                                                                                    <p className={`mt-1 text-xs text-white`}>{lang == 'ar' ? globalStore?.showroom_arabic : globalStore?.showroom}</p>
                                                                                    <p className={`mt-1 text-xs text-white`}>{lang == 'ar' ? globalStore?.showroom_address_arabic : globalStore?.showroom_address} | 8002444464</p>
                                                                                    <p className={`mt-1.5 text-xs font-bold text-white`}>{lang == 'ar' ? globalStore?.waybill_city_data?.name_arabic : globalStore?.waybill_city_data?.name}, {lang == 'ar' ? globalStore?.showroom_data?.store_regions?.name_arabic : globalStore?.showroom_data?.store_regions?.name} | {lang == 'ar' ? 'المملكة العربية السعودية' : 'Saudi Arabia'}</p>
                                                                                    <div className="mt-3">
                                                                                        <p
                                                ...
                                                <div className={`mt-2 text-[#004B7A] font-regular text-xs bg-white p-3 shadow-md rounded-md border ${requiredDate ? "border-[#219EBC]" : "border-red"}`}>
                                                    ...
                                                    <label className="text-xs font-semibold">{lang === 'ar' ? "تحديد موعد التوصيل" : "Schedule Delivery"}</label>
                                                    </div> */}
                                                    {/* Here Date Will Come */}
                                                                                                    ...
                                                                                                    <div className="flex items-center gap-x-3 w-full">
                                                                                                        ...
                                                                                                        <div className="w-full">
                                                                                                            <RadioGroup.Label as="p" className={`text-sm font-bold  mb-3 ${checked ? 'text-white' : 'text-[#004B7A]'}`}>
                                                                                                                ...
                                                                                                                {data?.make_default == 1 ?
                                                                                                                    <span className={`px-2 py-1 bg-[#219EBC30] text-xs rounded-sm text-[#219EBC] ml-2 rtl:mr-2 ${checked ? 'text-white bg-[#FFFFFF30]' : ''}`}>{lang == 'ar' ? 'العنوان الرئيسي' : 'Primary Address'}</span>
                                                                                                                    : null}
                                                                                                            </RadioGroup.Label>
                                                                                                            <p className={`mt-1 text-xs ${checked ? 'text-white' : 'text-[#5D686F]'}`}>{data?.address}</p>
                                                                                                            <p className={`mt-1.5 text-xs font-bold ${checked ? 'text-white' : 'text-[#5D686F]'}`}>{lang == 'ar' ? data?.state_data?.region?.name_arabic : data?.state_data?.region?.name}, {lang == 'ar' ? data?.state_data?.name_arabic : data?.state_data?.name} | {lang == 'ar' ? 'المملكة العربية السعودية' : 'Saudi Arabia'}</p>
                                                                                                            {data?.shippinginstractions ?
                                                                                                                <div className="mt-3">
                                                                                                                    <p className={`mt-3 text-xs font-bold ${checked ? 'text-white' : 'text-[#5D686F]'}`}>{lang == 'ar' ? 'تعليمات الشحن' : 'Shipping Instructions'}:</p>
                        ...
                        {activeTab3 === 2 ?
                            <div className='my-4'>
                                <span className="text-[#B15533] text-sm">{lang == 'ar' ? 'بيانات التواصل معك' : 'Contact information with you'}</span>
                                <div className="mt-2 border bg-white border-[#219EBC] rounded-md p-3">
                                    <div>
                                        <div className="flex items-center justify-between text-sm">
                                            <label className="font-regular text-[#5D686F]">{lang == 'ar' ? 'التواصل' : 'Communication'}</label>
                                        </div>
                                        ...
                                        return (
                                            <div key={addressData?.id + 1}>
                                                <div className="flex items-center justify-between text-sm">
                                                    ...
                                                    <button className="focus-visible:outline-none text-[#FF671F] hover:underline" onClick={() => setActiveTab3(1)}>{lang == 'ar' ? 'تغــيــيـر' : 'Change'}</button>
                                                </div>
                                                <div className="flex items-center gap-x-2 mt-1 rtl:mt-2 text-[#004B7A] fill-[#004B7A] font-regular text-xs">
                                                    ...
                                                    <div>
                                                        <p className="font-bold mb-1.5">{data.address_label == 0 ? lang == 'ar' ? 'الــمنـــزل' : 'Home' : lang == 'ar' ? 'مكتب' : 'Office'}</p>
                                                        <label className="">{data.address}</label>
                                                        <p className="font-medium text-xs">{lang == 'ar' ? data.state_data?.name_arabic : data.state_data?.name}, {lang == 'ar' ? data.state_data?.region?.name_arabic : data.state_data?.region?.name} | {lang == 'ar' ? 'المملكة العربية السعودية' : 'Saudi Arabia'}</p>
                                                    </div>
                                                </div>
                                                <hr className="opacity-10 my-3" />
                                            </div>
                                    ...
                                    <>
                                        <div key={globalStore?.id + 1}>
                                            <div className="align__center text-sm">
                                                <label className="font-regular text-[#5D686F]">{lang == 'ar' ? 'متجر الاستلام' : 'Pickup Store'}</label>
                                                {/* <button className="focus-visible:outline-none text-[#FF671F] hover:underline" onClick={() => setActiveTab3(1)}>{lang == 'ar' ? 'تغــيــيـر' : 'Change'}</button> */}
                                            </div>
                                            <div className="flex items-center gap-x-2 mt-1 rtl:mt-2 text-[#004B7A] fill-[#004B7A] font-regular text-xs">
                                                <svg id="fi_3514361" height="28" viewBox="0 0 256 256" width="28" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"><path d="m128 138.184a5 5 0 0 1 -3.607-1.538c-2.075-2.16-50.808-53.259-50.808-82.228a54.415 54.415 0 1 1 108.83 0c0 28.969-48.733 80.068-50.808 82.228a5 5 0 0 1 -3.607 1.538zm0-128.184a44.465 44.465 0 0 0 -44.415 44.418c0 19.07 29.312 54.978 44.414 71.451 15.1-16.478 44.416-52.4 44.416-71.451a44.465 44.465 0 0 0 -44.415-44.418z"></path><path d="m128 76.153a21.735 21.735 0 1 1 21.735-21.735 21.759 21.759 0 0 1 -21.735 21.735zm0-33.47a11.735 11.735 0 1 0 11.735 11.735 11.748 11.748 0 0 0 -11.735-11.735z"></path><path d="m128.126 256a4.992 4.992 0 0 1 -2.5-.67l-77.175-44.559a5 5 0 0 1 -2.5-4.331v-38.385a5 5 0 0 1 10 0v35.5l72.175 41.67 72.174-41.67v-35.88a5 5 0 0 1 10 0v38.765a5 5 0 0 1 -2.5 4.331l-77.174 44.556a4.992 4.992 0 0 1 -2.5.673z"></path><path d="m128.126 166.884a4.992 4.992 0 0 1 -2.5-.67l-77.175-44.557a5 5 0 1 1 5-8.66l74.675 43.113 74.674-43.11a5 5 0 1 1 5 8.66l-77.174 44.557a4.992 4.992 0 0 1 -2.5.667z"></path><path d="m160.933 198.291a5 5 0 0 1 -3.459-1.389l-32.806-31.402a5 5 0 0 1 6.916-7.224l30.1 28.813 68.154-39.349-27.558-26.382-27.359-15.744a5 5 0 1 1 4.988-8.667l27.885 16.047a4.988 4.988 0 0 1 .964.721l32.806 31.407a5 5 0 0 1 -.958 7.942l-77.174 44.557a4.993 4.993 0 0 1 -2.499.67z"></path><path d="m95.067 198.525a4.985 4.985 0 0 1 -2.5-.67l-77.173-44.555a5 5 0 0 1 -.957-7.942l33.057-31.642a4.967 4.967 0 0 1 .957-.718l27.634-15.955a5 5 0 1 1 5 8.66l-27.112 15.653-27.807 26.616 68.154 39.348 30.349-29.048a5 5 0 1 1 6.914 7.224l-33.058 31.641a4.991 4.991 0 0 1 -3.458 1.388z"></path></svg>
                                                <div>
                                                    <p className="font-medium text-xs">{lang == 'ar' ? globalStore?.showroom_arabic : globalStore?.showroom}</p>
                                                    <label className="">{lang == 'ar' ? globalStore?.showroom_address_arabic : globalStore?.showroom_address} | 8002444464</label>
                                                    <p className="font-medium text-xs">{lang == 'ar' ? globalStore?.waybill_city_data?.name_arabic : globalStore?.waybill_city_data?.name}, {lang == 'ar' ? globalStore?.showroom_data?.store_regions?.name_arabic : globalStore?.showroom_data?.store_regions?.name} | {lang == 'ar' ? 'المملكة العربية السعودية' : 'Saudi Arabia'}</p>
                                                </div>
                                            </div>
                                            {cartData?.storeType == 0 && cartData?.storeId == false && (
                                                <hr className="opacity-10 my-3" />
                                            ...
                                            <div className="w-full p-3 rounded-b-md bg-white">
                                                ...
                                                <div
                                                    ...
                                                    <span className="flex items-center gap-1 text-primary peer-checked:text-[#1c262d]">
                                                        {lang === "ar" ? "استخدم رصيدي" : "Use My"}{' '}{Number(usableloyaltyAmount)?.toFixed(2)}{''}<svg className="riyal-svg shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1124.14 1256.39" width="13" height="13"><path fill="currentColor" d="M699.62,1113.02h0c-20.06,44.48-33.32,92.75-38.4,143.37l424.51-90.24c20.06-44.47,33.31-92.75,38.4-143.37l-424.51,90.24Z"></path><path fill="currentColor" d="M1085.73,895.8c20.06-44.47,33.32-92.75,38.4-143.37l-330.68,70.33v-135.2l292.27-62.11c20.06-44.47,33.32-92.75,38.4-143.37l-330.68,70.27V66.13c-50.67,28.45-95.67,66.32-132.25,110.99v403.35l-132.25,28.11V0c-50.67,28.44-95.67,66.32-132.25,110.99v525.69l-295.91,62.88c-20.06,44.47-33.33,92.75-38.42,143.37l334.33-71.05v170.26l-358.3,76.14c-20.06,44.47-33.32,92.75-38.4,143.37l375.04-79.7c30.53-6.35,56.77-24.4,73.83-49.24l68.78-101.97v-.02c7.14-10.55,11.3-23.27,11.3-36.97v-149.98l132.25-28.11v270.4l424.53-90.28Z"></path></svg> {lang === "ar" ? 'تمكين نخب' : 'Tamkeen Nukhub'}
                                                    </span>
                                                    </label>
                                                </div>
                                                {displayText ?
                            ...
                            <div className='my-4'>
                                ...
                                :null}
                                    ...
                                    {doorStepDataStatus ?
                                        <>
                                            <div className="text-sm font-medium">
                                                <label className="font-regular text-[#5D686F]">{lang == 'ar' ? doorStepData?.title_arabic : doorStepData?.title}</label>
                                                    ...
                                                    {/* <label>
                                                        {
                                                            paymentMethod == 'tamara' ? lang == 'ar' ? `غدا تقسيط علي 12 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 12)} ريال في الشهر` : `Installments for 12 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 12)} per month`
                                                                : paymentMethod == 'tabby' ? lang == 'ar' ? `غدا تقسيط علي 4 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} ريال في الشهر` : `Installments for 4 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} per month`
                                                                    : paymentMethod == 'madfu' ? lang == 'ar' ? `غدا تقسيط علي 4 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} ريال في الشهر` : `Installments for 4 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} per month`
                                                                        : paymentMethod == 'mispay' ? lang == 'ar' ? `غدا تقسيط علي 4 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} ريال في الشهر` : `Installments for 4 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} per month`
                                                                            : paymentMethod == 'clickpay' ? lang == 'ar' ? `غدا تقسيط علي 4 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} ريال في الشهر` : `Installments for 4 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 4)} per month`
                                                                                : paymentMethod == 'tasheel' ? lang == 'ar' ? `غدا تقسيط علي 36 شهر بمبلغ ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 36)} ريال في الشهر` : `Installments for 36 months at an amount of ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price / 36)} per month`
                                                                                    : paymentMethod == 'madapay' ? lang == 'ar' ? `الدفع النقدي ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}` : `Instant pay ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}`
                                                                                        : paymentMethod == 'applepay' || paymentMethod == 'clickpay_applepay' ? lang == 'ar' ? `الدفع النقدي ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}` : `Instant pay ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}`
                                                                                            : paymentMethod == 'hyperpay' ? lang == 'ar' ? `الدفع النقدي ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}` : `Instant pay ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}`
                                                                                                : paymentMethod == 'cod' ? lang == 'ar' ? `الدفع عند الاستلام ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}` : `Paid upon delivery ${Intl.NumberFormat('en-US').format(summary?.filter((element: any) => element.key == 'total')[0]?.price)}`
                                                                                                    : null
                                                        }
                                                    </label>
                                                    {currencySymbol} */}
                                    ...
                                    <div className="mt-2 text-[#004B7A] font-regular text-xs bg-white p-3 shadow-md rounded-md border border-[#219EBC] flex items-center justify-between gap-x-4">
                                        <div className="flex items-center gap-x-4">
                                            ...
                                            <div className="font-bold">
                                                <h6 className="text-[#004B7A] text-sm">{lang == 'ar' ? doorStepData?.title_arabic : doorStepData?.title}</h6>
                                                <span className="text-xs font-medium text-[#5D686F]">{lang == 'ar' ? 'من المتوقع أن يتم التسليم يوم' : 'Delivery is expected to take place on'} <span className="font-bold">{dayjs().add(2, 'days').locale(lang == 'ar' ? 'ar' : 'en').format("MMM  DD, YYYY")}</span></span>
                    ...
                    <div className="md:w-1/2 w-full max-md:pb-10">
                        <h3 className='text-base mb-1 md:text-lg font-bold md:mb-3'>{lang == 'ar' ? 'تفاصيل الطلب الخاص بك' : 'Checkout Summary'}</h3>
                                ...
                                if (s?.key == 'total') {
                                    return (
                                        <React.Fragment key={`total-${i}`}>
                                            ...
                                            <span className='text-sm md:text-base font-bold text-orangePrice'>{lang === 'ar' ? 'جميع أسعار المنتجات شاملة الضريبة المضافة' : 'All product prices included with VAT'}</span>
                                        </React.Fragment>
                                    )
                                }
                                return (
                                    <div className="flex items-center justify-between text-sm font-medium mb-3" key={i + 4}>
                                        <h6 className={`text-[#1C262D] capitalize`}>{lang === 'ar' ? s?.title_arabic : s?.title}</h6>
                        ...
                        {activeTab3 === 1 || activeTab3 === 2 ?
                            ...
                            <div className="space-y-3 mt-3 py-4">
                                <h3 className='text-base md:text-lg font-bold'>{lang == 'ar' ? 'قائمة المنتجات الخاصة بك' : 'Products'}</h3>
                                <span className="text-[#5D686F] text-xs font-light">{lang == 'ar' ? `لديك (${checkoutData?.products?.length}) منتجات في العربة` : `You have (${checkoutData?.products?.length}) products in the cart`}</span>
                                {checkoutData?.products?.map((pro: any, i: number) => {
                                    ...
                                    return (
                                        <React.Fragment key={i}>
                                            ...
                                            <div className="bg-white rounded-md shadow-md mb-4 pb-2" key={pro.id}>
                                                <div className='flex items-center gap-x-4 relative'>
                                                    <div>
                                                        <Image
                                                            src={pro.image}
                                                            alt={lang == 'ar' ? pro.name_arabic : pro.name}
                                                            title={lang == 'ar' ? pro.name_arabic : pro.name}
                                                            height={100}
                                                            width={100}
                                                            loading='lazy'
                                                    ...
                                                    {pro.regular_price > pro.price ? <div className="text-[#EA4335] text-xs absolute ltr:left-0 rtl:right-0 top-0 bg-[#EA433520] px-3.5 py-1 rtl:rounded-bl-lg ltr:rounded-br-lg ltr:rounded-tl-lg flex gap-1">{lang == 'ar' ? Math.round(((pro.regular_price - pro.price) * 100) / pro.regular_price) + '% خصم' : Math.round(((pro.regular_price - pro.price) * 100) / pro.regular_price) + '% OFF'}</div> : null}
                                                    <div className="flex items-end justify-between w-full p-3">
                                                        <div>
                                                            {pro.pre_order ?
                                                                <>
                                                                    ...
                                                                    <p className="text-sm text-primary font-medium">{lang === 'ar' ? 'سلمت بواسطة' : 'Delivered by'}: {dayjs().add(pro.pre_order_day, 'days').locale(lang == 'ar' ? 'ar' : 'en').format("MMM DD, YYYY")}</p>
                                                                </>
                                                                : null}
                                                            <h4 className="text-primary text-xs line-clamp-2">{lang == 'ar' ? pro.name_arabic : pro.name}</h4>
                                                        ...
                                                        <div className="flex items-center gap-x-2 text-xs">
                                                            <span className="font-medium">{lang == 'ar' ? 'عدد' : 'Qty'}:</span>
                                                        ...
                                                        <>
                                                            <div className='flex items-center gap-x-4 mx-2 mt-2 bg-[#fde18d] rounded-md p-2 w-[-webkit-fit-content]'>
                                                                ...
                                                                <div className='text-sm font-normal'>
                                                                    <h6 className='font-bold text-base'>{lang == "ar" ? "احصل عليه في غضون 24 الى 48 ساعة" : "Get it in 24 to 48 hours"}</h6>
                                                                    <p className=''>
                                                                        {lang === 'ar'
                                        ...
                                                        </>
                                                    ...
                                                    <div className='flex items-center gap-x-4'>
                                                        ...
                                                        <div className='text-xs font-normal'>
                                                            <h6 className='font-semibold text-sm'>{lang == "ar" ? "ومن المتوقع أن يتم التسليم في " : "Delivery is expected to take place on " + dayjs().add(1, 'days').format('DD MMM')} {dayjs().add(5, 'days').format('DD MMM')}</h6>
                                                            ...
                                                            <div className={`bg-[#EEF8FC] shadow-md rounded-md p-3`} key={i * 4}>
                                                                <div className="flex items-center gap-x-3">
                                                                    <Image
                                                                        ...
                                                                        alt={giftData?.image ? lang == 'ar' ? giftData?.name_arabic : giftData?.name : ''}
                                                                        title={giftData?.image ? lang == 'ar' ? giftData?.name_arabic : giftData?.name : ''}
                                                                        height={65}
                                                                        width={65}
                                                                        loading='lazy'
                                                                        className="rounded-md"
                                                                    />
                                                                    <div>
                                                                        <h3 className="text-[#004B7A] mt-3 text-xs text-left rtl:text-right line-clamp-2">{lang == 'ar' ? giftData?.name_arabic : giftData?.name}</h3>
                                                                        <div className="flex items-center justify-between">
                                                                            ...
                                                                            <h6 className="font-normal mt-1 text-xs text-left rtl:text-right">{lang == 'ar' ? 'عدد' : 'Qty'}: {giftData?.quantity}</h6>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <hr className='my-2 opacity-5' />
                                                                {(pro.express && pro?.express_qty >= pro?.quantity) && giftData.express && giftData?.express_qty >= giftData?.quantity ?
                                                                    ...
                                                                    <div className='flex items-center gap-x-4'>
                                                                        <svg id="fi_10112476" height="35" viewBox="0 0 512 512" width="40" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"><g fillRule="evenodd"><path d="m310.168 100.287c6.255 0 11.357 5.103 11.357 11.357v261.113h-264.358c-12.456 0-22.62-10.162-22.62-22.62v-238.494c0-6.254 5.102-11.357 11.357-11.357h264.264z" fill="#323e66"></path><path d="m321.525 372.757h-264.358c-12.459 0-22.62-10.162-22.62-22.62v-38.283h286.978v60.904z" fill="#e37500"></path><path d="m483.254 372.757h-161.729v-186.34h97.774c7.146 0 13.298 2.704 18.13 7.97l39.349 42.88c4.331 4.72 6.477 10.231 6.477 16.637v118.854z" fill="#f9ac00"></path><path d="m467.005 372.757h-145.48v-186.34h81.524c7.147 0 13.298 2.704 18.13 7.97l39.349 42.88c4.331 4.72 6.477 10.231 6.477 16.637v118.854z" fill="#fdc72e"></path><path d="m368.851 257.715v-52.102h78.88l29.046 31.653c4.331 4.72 6.477 10.231 6.477 16.637v3.812z" fill="#e9e9ff"></path><path d="m368.851 257.715v-52.102h62.63l29.047 31.653c4.331 4.72 6.477 10.231 6.477 16.637v3.812h-98.153z" fill="#f0f0ff"></path><path d="m483.254 350.966h-448.708v21.79h450.364c8.861 0 16.089-7.227 16.089-16.089v-29.336h-17.745z" fill="#7986bf"></path></g><path d="m34.546 332.788h23.169v18.178h-23.169z" fill="#323e66"></path><path d="m460.89 291.132h22.364v25.908h-22.364z" fill="#323e66"></path><circle cx="396.154" cy="372.757" fill="#323e66" r="38.953"></circle><path d="m396.154 391.897c10.542 0 19.14-8.598 19.14-19.14s-8.598-19.14-19.14-19.14-19.14 8.598-19.14 19.14 8.598 19.14 19.14 19.14z" fill="#fdc72e" fillRule="evenodd"></path><circle cx="118.375" cy="372.757" fill="#323e66" r="38.953" transform="matrix(.987 -.162 .162 .987 -58.875 24.127)"></circle><path d="m118.375 391.897c10.542 0 19.14-8.598 19.14-19.14s-8.598-19.14-19.14-19.14-19.14 8.598-19.14 19.14 8.598 19.14 19.14 19.14z" fill="#fdc72e" fillRule="evenodd"></path><path d="m152.024 235.62h76.233v76.233h-76.233z" fill="#e6b17c"></path><path d="m178.996 235.62h22.289v24.799l-11.145-5.593-11.144 5.593z" fill="#fdc72e" fillRule="evenodd"></path><path d="m152.024 159.387h76.233v76.233h-76.233z" fill="#ba8047"></path><path d="m178.996 159.387h22.289v24.799l-11.145-5.593-11.144 5.593z" fill="#2dd62d" fillRule="evenodd"></path><path d="m228.257 235.62h76.233v76.233h-76.233z" fill="#dea368"></path><path d="m255.228 235.62h22.289v24.799l-11.144-5.593-11.145 5.593z" fill="#fb545c" fillRule="evenodd"></path><path d="m228.257 135.311h76.233v100.309h-76.233z" fill="#fdd7ad"></path><path d="m255.228 135.311h22.289v24.799l-11.144-5.593-11.145 5.593z" fill="#5caeff" fillRule="evenodd"></path><path d="m75.791 235.62h76.233v76.233h-76.233z" fill="#fdd7ad"></path><path d="m102.763 235.62h22.289v24.799l-11.145-5.593-11.144 5.593z" fill="#5caeff" fillRule="evenodd"></path><path d="m321.525 186.416h93.116l-15.238-15.238c-2.093-2.093-4.64-3.148-7.599-3.148h-70.279z" fill="#e37500" fillRule="evenodd"></path><path d="m75.791 235.62h8.362v76.233h-8.362z" fill="#f2c496"></path><path d="m152.024 235.62h8.362v76.233h-8.362z" fill="#dea368"></path><path d="m228.257 235.62h8.362v76.233h-8.362z" fill="#d19458"></path><path d="m228.257 135.311h8.362v100.309h-8.362z" fill="#f2c496"></path><path d="m152.024 159.387h8.362v76.233h-8.362z" fill="#ab733a"></path><path d="m56.692 187.026h-41.059c-2.762 0-5-2.238-5-5s2.238-5 5-5h41.059c2.757 0 5 2.238 5 5s-2.243 5-5 5zm21.777 26.773h-57.117c-2.762 0-5-2.238-5-5s2.238-5 5-5h57.116c2.762 0 5 2.238 5 5s-2.238 5-5 5zm-48.188-67.832h33.202c2.757 0 5 2.238 5 5s-2.243 5-5 5h-33.202c-2.762 0-5-2.238-5-5s2.238-5 5-5zm-19.282-12.843c-2.757 0-5-2.243-5-5s2.243-5 5-5h42.84c2.762 0 5 2.238 5 5s-2.238 5-5 5zm107.371 253.768c-7.791 0-14.139-6.338-14.139-14.139s6.348-14.139 14.139-14.139 14.139 6.348 14.139 14.139-6.338 14.139-14.139 14.139zm0-38.278c-13.31 0-24.139 10.829-24.139 24.139s10.829 24.139 24.139 24.139 24.139-10.829 24.139-24.139-10.829-24.139-24.139-24.139zm277.778 38.278c-7.791 0-14.139-6.338-14.139-14.139s6.348-14.139 14.139-14.139 14.143 6.348 14.143 14.139-6.343 14.139-14.143 14.139zm0-38.278c-13.305 0-24.139 10.829-24.139 24.139s10.834 24.139 24.139 24.139 24.144-10.829 24.144-24.139-10.834-24.139-24.144-24.139zm-37.168-51.288c0-2.762 2.229-5 5-5h21.491c2.757 0 5 2.238 5 5s-2.243 4.995-5 4.995h-21.491c-2.772 0-5-2.224-5-4.995zm137.02 59.336c0 6.124-4.981 11.091-11.091 11.091h-45.097c-.472-4.129-1.51-8.076-3.043-11.786h46.483c2.767 0 5-2.243 5-5v-18.634h7.748zm-99.851 50.054c18.72 0 33.963-15.244 33.963-33.963s-15.244-33.949-33.963-33.949-33.949 15.229-33.949 33.949 15.234 33.963 33.949 33.963zm-277.778 0c18.729 0 33.959-15.244 33.959-33.963s-15.229-33.949-33.959-33.949-33.949 15.229-33.949 33.949 15.229 33.963 33.949 33.963zm-78.817-50.75h38.206c-1.528 3.71-2.576 7.657-3.048 11.786h-35.159v-11.786zm13.167-10h-13.167v-8.181h13.167zm263.801-29.111v29.111h-163.34c-8.039-10.434-20.648-17.163-34.811-17.163s-26.758 6.729-34.801 17.163h-20.849v-13.181c0-2.762-2.238-5-5-5h-18.167v-10.929h276.967zm-235.734-76.236h16.972v19.8c0 1.733.9 3.343 2.381 4.253.8.495 1.71.748 2.619.748.772 0 1.538-.176 2.248-.533l8.9-4.467 8.9 4.467c1.552.776 3.391.7 4.872-.214 1.471-.909 2.372-2.519 2.372-4.253v-19.8h16.967v66.236h-66.231v-66.236zm26.972 0v11.695l3.9-1.957c1.419-.705 3.081-.705 4.491 0l3.9 1.957v-11.695zm49.259-76.232h16.982v19.801c0 1.733.89 3.343 2.367 4.252 1.471.91 3.324.99 4.872.214l8.901-4.467 8.9 4.467c.709.357 1.481.529 2.238.529.924 0 1.833-.248 2.633-.743 1.481-.91 2.367-2.519 2.367-4.252v-19.801h16.981v66.231h-66.241zm26.982 0v11.696l3.9-1.957c1.41-.71 3.067-.71 4.481 0l3.895 1.957v-11.696h-12.277zm66.231-24.077v19.801c0 1.733.896 3.343 2.367 4.252 1.481.91 3.319.99 4.872.214l8.9-4.467 8.896 4.467c.714.357 1.481.533 2.252.533.91 0 1.819-.253 2.619-.748 1.481-.91 2.381-2.519 2.381-4.252v-19.801h16.972v90.308h-66.231v-90.308h16.972zm9.996 0v11.696l3.9-1.957c1.409-.71 3.071-.71 4.491 0l3.9 1.957v-11.696zm0 100.309h12.291v11.695l-3.9-1.957c-1.419-.705-3.081-.705-4.491 0l-3.9 1.957zm-83.861 24.053c1.471.91 3.324.991 4.872.214l8.901-4.467 8.9 4.467c.709.357 1.481.533 2.238.533.924 0 1.833-.252 2.633-.748 1.481-.909 2.367-2.519 2.367-4.253v-19.8h16.981v66.236h-66.241v-66.236h16.982v19.8c0 1.733.89 3.343 2.367 4.253zm7.634-24.053v11.695l3.9-1.957c1.41-.705 3.067-.705 4.481 0l3.895 1.957v-11.695h-12.277zm49.259 66.236h66.231v-66.236h-16.972v19.8c0 1.733-.9 3.343-2.381 4.253-1.467.914-3.319.991-4.872.214l-8.896-4.467-8.9 4.467c-.709.357-1.471.533-2.243.533-.91 0-1.819-.252-2.629-.748-1.471-.909-2.367-2.519-2.367-4.253v-19.8h-16.972v66.236zm158.54-133.825c1.629 0 2.919.533 4.067 1.686l6.7 6.7h-76.046v-8.386zm-232.81 182.936h196.551c-1.543 3.71-2.581 7.657-3.052 11.786h-190.46c-.466-4.129-1.509-8.076-3.038-11.786zm274.75-158.202c-3.862-4.214-8.719-6.348-14.439-6.348h-92.78v154.549h34.83c8.039-10.434 20.648-17.163 34.797-17.163s26.772 6.729 34.811 17.163h47.292v-23.925h-17.363c-2.757 0-5-2.238-5-4.995v-25.911c0-2.762 2.243-5 5-5h17.363v-23.42h-109.4c-2.762 0-5-2.238-5-5v-52.102c0-2.762 2.238-5 5-5h67.508zm44.459 54.95h-104.347v-42.102h71.679l27.558 30.034c3.2 3.486 4.872 7.448 5.11 12.067zm.052 59.331h-12.362v-15.91h12.362zm22.749 10.286h-12.748v-68.427c0-7.653-2.624-14.386-7.791-20.02l-39.354-42.878c-5.748-6.272-13.296-9.591-21.806-9.591h-2.591l-13.772-13.777c-3.019-3.014-6.867-4.61-11.139-4.61h-65.279v-51.388c0-9.019-7.333-16.358-16.353-16.358h-277.019c-2.757 0-5 2.243-5 5s2.243 5 5 5h277.021c3.505 0 6.353 2.852 6.353 6.357v195.213h-7.034v-171.543c0-2.757-2.238-5-5-5h-76.227c-2.762 0-5 2.243-5 5v19.077h-71.241c-2.757 0-5 2.238-5 5v71.231h-71.232c-2.757 0-4.995 2.238-4.995 5v71.236h-31.239v-78.556c0-2.762-2.243-5-5-5-2.772 0-5 2.238-5 5v144.454c0 2.762 2.229 5 5 5h40.159c2.5 21.891 21.12 38.964 43.659 38.964s41.168-17.072 43.659-38.964h190.461c2.49 21.891 21.12 38.964 43.659 38.964s41.173-17.072 43.664-38.964h45.097c11.629 0 21.091-9.457 21.091-21.091v-29.33c0-2.757-2.238-5-5-5z" fillRule="evenodd"></path></svg>
                                                                        <div className='text-xs font-normal'>
                                                                            <h6 className='font-semibold text-sm'>{lang == "ar" ? "احصل عليه خلال 24 ساعة" : "Delivery is expected to take place on " + dayjs().add(1, 'days').format('DD MMM')} {dayjs().add(5, 'days').format('DD MMM')}</h6>
                                                                            <p>{lang == "ar" ? `فقط ${3} متاحة خلال 24 ساعة للتوصيل السريع` : `Order in 1 h 4 m`}</p>
                                                                        </div>
                                                                    </div>
                                                                }
                                                            </div>
                                                            ...
                                                            <div className={`bg-[#EEF8FC] shadow-md rounded-md p-3 ${pro?.fbt?.length > 1 ? 'max-md:w-72' : 'w-full'}`} key={i * 6}>
                                                                <div className="flex items-center gap-x-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <h3 className="text-[#004B7A] mt-3 text-xs text-left rtl:text-right line-clamp-2">{lang == 'ar' ? giftData?.name_arabic : giftData?.name}</h3>
                                                                        <div className="flex items-center justify-between">
                                                                            <p className="text-[#EA4335] text-sm">{lang == 'ar' ? pro.discounted_amount.toLocaleString('EN-US') + ' ر.س' : 'SR ' + giftData?.discounted_amount.toLocaleString('EN-US')}{'  '}</p>
                                                                            <h6 className="font-normal mt-1 text-xs text-left rtl:text-right">{lang == 'ar' ? 'عدد' : 'Qty'}: {giftData?.quantity}</h6>
                                                                        </div>
                                                                    </div>
                                                                </div>
            ...
            {addressData?.length ?
                <div className="fixed bottom-0 w-full p-3 bg-white shadow-md border-t border-[#5D686F26] z-10">
                        ...
                        <>
                            {/* <div className="text-xs pt-1 flex items-center">
                                <label htmlFor="termsandConditionseid">
                                    <span className="text-xs text-[#DC4E4E] decoration-[#DC4E4E] font-medium">{lang === "ar" ? "عميلنا العزيز: يرجى العلم انه قد يكون بعض التأخير في التوصيل بسبب التزامن مع إجازة عيد الأضحى المبارك" : "Dear Customer: Kindly note that due to Eid holidays there might be a delay in the deliveries."}</span>
                                </label>
                            </div> */}
                            <div className="text-xs pt-1 flex items-center mb-3">
                                ...
                                <label htmlFor="termsandConditions">
                                    <span>{lang === 'ar' ? 'عزيزي العميل: بالضغط على هذا الزر فإنك توافق على' : `Dear Customer: By clicking on this button you're agreeing with our`}{' '}</span>
                                    <span className="font-bold underline text-[#004B7A]"><Link href={`${origin}/${lang}/terms-and-conditions`}>{lang === 'ar' ? 'الشروط والأحكام' : `Terms & Condition.`}</Link></span>
                                    <span>{' '}{lang === 'ar' ? 'الخاصة بنا.' : ""}</span>
                                </label>
                            </div>
                        </>
                        : null}
                    <button
                        ...
                        {activeTab3 === 1 ? lang == 'ar' ? 'الانتقال الي الدفع' : 'Proceed to Payment' : ''}
                        {activeTab3 === 2 ? lang == 'ar' ? 'ادفع الان' : 'Review Order' : ''}
                        {activeTab3 === 3 && !ProceesBtn ? lang == 'ar' ? 'اكمال الطلب' : 'Checkout Now' : ''}
                        ...
                        {/* {activeTab3 === 3 ? lang == 'ar' ? 'اكمال الطلب' : 'Checkout Now' : ''} */}
                    </button>
                </div>
                : null}
        </>
