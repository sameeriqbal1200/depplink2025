...
export default async function TabbyLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Tabby"
            : footer?.data?.meta_title_ar ?? "تابي  | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Tabby"
            : footer?.data?.meta_description_ar ??
            "تابي معارض تمكين";

    // In /[lang]/Tabby/[...slug], slugParts are ONLY the [...slug] bits (not "Tabby")
