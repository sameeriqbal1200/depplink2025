...
export default async function ReturnandExchangeLayout({ children }: { children: React.ReactNode }) {
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Return Exchange"
            : footer?.data?.meta_title_ar ?? "الإرجاع والاستبدال | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Return Exchange"
            : footer?.data?.meta_description_ar ??
            "معارض تمكين الإرجاع والاستبدال";

    // In /[lang]/returnexchange/[...slug], slugParts are ONLY the [...slug] bits (not "returnexchange")
