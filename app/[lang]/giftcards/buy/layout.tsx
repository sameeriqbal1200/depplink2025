...
export default async function GiftCardBuyLayout({ children }: { children: React.ReactNode }) {
    const { lang, baseUrl } = await getRequestContext();
     const slugStr = "giftcards-buy";
    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === 'ar' ? 'بطاقات الهدايا' : 'Gift Cards',
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Gift Cards"
            : footer?.data?.meta_title_ar ?? "بطاقات هدايا تمكين ستورز";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ?? "Tamkeen Stores Gift Cards"
            : footer?.data?.meta_description_ar ?? "بطاقات هدايا تمكين ستورز";
